...
#define HEVOS_FAT16_SIGNATURE 0x29
#define HEVOS_FAT16_FAT_ENTRY_SIZE 0x02
#define HEVOS_FAT16_BAD_SECTOR 0xFF7
#define HEVOS_FAT16_UNUSED 0x00
...
struct fat_private
{
    ...
    // Used to stream the file allocation table (fat)
...
int fat16_write(struct disk* disk, void* descriptor, uint32_t size, uint32_t nmemb, char* buf);
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != HEVOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != HEVOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != HEVOS_ALL_OK)
    {
        res = -EIO;
        goto error_out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != HEVOS_ALL_OK)
    {
        res = -EIO;
        goto error_out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != HEVOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != HEVOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * HEVOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < cluster_ahead; i++)
    {
        ...
        {
            // We are at the end of the sub directory
            res = -EIO;
            goto out;
        }

        //check if sector is marked as bad
        if (entry == HEVOS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
static int fat16_write_internal_to_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* buf)
{
    int res = 0;
    ...
    res = diskstreamer_write(stream, buf, total_to_write);
    if (res != HEVOS_ALL_OK)
    {
        goto out;
    }
    ...
    if (total > 0)
    {
        ...
        res = fat16_write_internal_to_stream(disk, stream, cluster, offset + total_to_write, total, buf + total_to_write);
    }
...
static int fat16_write_internal(struct disk* disk, int starting_cluster, int offset, int total, void* buf)
{
    struct fat_private* fs_private = disk->fs_private;
    struct disk_stream* stream = fs_private->cluster_write_stream;
    return fat16_write_internal_to_stream(disk, stream, starting_cluster, offset, total, buf);
}
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[HEVOS_MAX_PATH];
...
int fat16_write(struct disk* disk, void* descriptor, uint32_t size, uint32_t nmemb, char* buf)
{
    ...
    for (uint32_t i = 0; i < nmemb; i++)
    {
        res = fat16_write_internal(disk, fat16_get_first_cluster(item), offset, size, buf);
        if (ISERR(res))
        {
            goto out;
        }
