...
struct filesystem* filesystems[HEVOS_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[HEVOS_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_file_system()
{
    int i = 0;
    for (i = 0; i < HEVOS_MAX_FILESYSTEMS; i++)
    {
        if (filesystems[i] == 0)
        {
...
{
    int res = -ENOMEM;
    for (int i = 0; i < HEVOS_MAX_FILE_DESCRIPTORS; i++)
    {
...
{
    if (fd <= 0 || fd >= HEVOS_MAX_FILE_DESCRIPTORS)
    {
        return 0;
    }
...
struct filesystem* fs_resolve(struct disk* disk) 
{
    ...
    for (int i = 0; i < HEVOS_MAX_FILESYSTEMS; i++)
    {
