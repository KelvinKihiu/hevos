...
{
    int len = strnlen(filename, HEVOS_MAX_PATH);
    return (len >= 3 && is_digit(filename[0]) && memcmp((void*)&filename[1], ":/", 2) == 0);
}

static int pathparser_path_get_drive_by_path(const char** path)
{
    ...
    // Add 3 bytes to skip drive number i.e. 0:/
    *path += 3;
...
{
    char* result_path_part = kzalloc(HEVOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > HEVOS_MAX_PATH)
    {
        goto out;
    }

    res = pathparser_path_get_drive_by_path(&temp_path);
    if (res < 0)
    {
        goto out;
    }
