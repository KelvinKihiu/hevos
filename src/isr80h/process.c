...
{
    void* file_name_user_ptr = task_get_stack_item(task_current(), 0);
    char filename[HEVOS_MAX_PATH];
    int res = copy_string_from_task(task_current(), file_name_user_ptr, filename, sizeof(filename));
    if (res < 0)
    {
        goto out;
    }

    char path[HEVOS_MAX_PATH];
    strcpy(path, "0:/");
...
{
    int error_code = (int) task_get_stack_item(task_current(), 0);
    struct process* process = task_current()->process;
