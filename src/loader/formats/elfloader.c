...
{
    return header->e_type == ET_EXEC && header->e_entry >= HEVOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? HEVOS_ALL_OK : -EINFORMAT;
}
