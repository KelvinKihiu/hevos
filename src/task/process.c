...
static struct process* processes[HEVOS_MAX_PROCESSES] = {};
...
{
    if (process_id < 0 || process_id >= HEVOS_MAX_PROCESSES)
    {
        return NULL;
    }
...
{
    int res = -ENOMEM;
    for (int i = 0; i < HEVOK_MAX_PROGRAM_ALLOCATIONS; i++)
    {
...
{
    for (int i = 0; i < HEVOS_MAX_PROCESSES; i++)
    {
        if (processes[i])
        {
...
{
    // Remove link in the page table [Here]
...
{
    int res = 0;
    paging_map_to(process->task->page_directory, (void*) HEVOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
int process_map_memory(struct process* process)
{
    ...
    // Map the stack
    paging_map_to(process->task->page_directory, (void*)HEVOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack + HEVOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(HEVOS_USER_PROGRAM_STACK_SIZE);
