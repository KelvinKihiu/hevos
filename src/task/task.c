...
int task_init(struct task* task, struct process* process)
{
    ...
    task->registers.ip = HEVOS_PROGRAM_VIRTUAL_ADDRESS;
    if (process->filetype == PROCESS_FILETYPE_ELF)
    {
        task->registers.ip = elf_header(process->elf_file)->e_entry;
    }
    task->registers.ss = HEVOS_USER_DATA_SEGMENT;
    task->registers.cs = HEVOS_USER_CODE_SEGMENT;
    task->registers.esp = HEVOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
